...
from settings import OBJECT_IMAGE_INCREMENT, screen
...
class Object(ABC):
    ...
    def __init__(self, path_to_sheet, x, y):  # x, y -> placement
        self.object_sheet = pygame.image.load(
            path_to_sheet
        )  # Using .convert() would disable pixel-perfect collisions
    ...
    def get_object_image(
        self, width, height, scale, color, row_number, number_of_images, list_of_images
    ):
        """
...
class InteractableObject(Object, SoundAssets):
    ...
    def object_animation(self):
        if (
            not self.object_is_usable and self.number_of_interactions > 0
        ):  # if object is not usable, then it is being used
...
class DangerousObject(Object, SoundAssets):
    ...
    def object_animation(self):
        ...
        if not self.delay_finished:
            if (
                pygame.time.get_ticks() - self.animation_start_time
        ...
        if self.dx == 0:
            ...
            if self.direction == -1:  # going left
                obj_surface = pygame.transform.flip(
                    obj_surface, True, False
                ).convert_alpha()
            screen.blit(obj_surface, self.obj_rect)
