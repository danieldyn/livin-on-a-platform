...
- **Storage** - persistently stores all level highscores and the most recently completed level.
...
- Querying **Storage** to obtain the data that needs to be compared and perhaps overwritten in certain situations.
...
- Score and Time: Tracks the player's score (based on collectables) and the time elapsed for the current level, all handled inside `levels.py` in separate methods.
...
&nbsp; &nbsp; &nbsp; &nbsp; This is where important data from different playing sessions can remain persistently, giving the game a sense of continuity and progress. For ease of parsing and writing back, JSON files will be used, being updated dynamically whenever the player beats a level's highscore (for the dedicated Feats Menu with all highscores) or completes a level (for autosaving). The local saving feature effect can be reverted using a Reset button, allowing a "New Save" to be created by overwriting the existing one. In case the player plays several levels, return to the main menu and expects to see the changes taking place, the files are reloaded whenever the "Main Menu" state is entered.
