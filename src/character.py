...
from settings import (
    BLOCK_SIZE,
    IDLE_IMAGE_INCREMENT,
    ROLLING_IMAGE_INCREMENT,
    RUNNING_IMAGE_INCREMENT,
    SCREEN_HEIGHT,
    SCREEN_WIDTH,
    screen,
)
...
class Player(SoundAssets):
    ...
    def __init__(self, x, y):
        ...
        self.visual_sink_y = (
            6  # helps keep the feet of the character on the ground, visually speaking
        )
        ...
        self.draw_offset_y = (
            self.player_height - self.hitbox_height
        ) - self.visual_sink_y
        # image lists
        ...
        self.player_rect = pygame.rect.Rect(
            self.starting_x, self.starting_y, self.hitbox_width, self.hitbox_height
        )
        self.can_jump = True
    ...
    def render_image(self):
        ...
        if self.dx == 0 and self.dy == 0 and self.can_jump:
            ...
            if self.idle_img_index >= len(self.idle_image_list):
                self.idle_img_index = 0
            if (
                self.player_is_rolling
            ):  # if the player is stuck under a block (when rolling)
            ...
            if self.dx < 0:  # flip the image, making the player "turn"
                img_frame = pygame.transform.flip(
                    img_frame, True, False
                ).convert_alpha()
        ...
        if current_time - self.last_hit_time < self.hit_cooldown:
            if (current_time // 200) % 2 == 0:
                if self.player_is_rolling:
                    screen.blit(
                        img_frame, (draw_pos_x, draw_pos_y - 13)
                    )  # found 13 by trial and error
                else:
    ...
    def get_movement_input(self, keys, level_world):
        ...
        wants_to_roll = keys[pygame.K_DOWN] and (
        ...
        if not wants_to_roll and self.player_is_rolling:
            ...
            growth_test_rect.bottom = (
                self.player_rect.bottom - 1
            )  # avoid checking the floor
    ...
    def handle_interactions(self, level_world, keys):
        ...
        """
        img_mask = self.running_img_list[int(self.running_img_index)][
            2
                ...
                if img_mask.overlap(obj_mask, (offset_x, offset_y)):
                    ...
                    if isinstance(obj, CollectableObject):
                        if obj.object_is_usable:
                            ...
                            if isinstance(obj, Coin):
                                self.coins_collected += (
                                    1  # avoid point farming after collecting the coin
                                )
                        ...
                        if keys[pygame.K_RETURN]:
                            if (
                                obj.object_is_usable and obj.number_of_interactions > 0
                            ):  # you can try using it
                                obj.object_is_usable = False
                                if obj.sound is not None:
                                    obj.sound.play()
                                if isinstance(
                                    obj, EndOfLevelObject
                                ):  # end of level flag
    ...
    def get_img(
        self, sheet, width, height, color, row_number, number_of_images, list_of_images
    ):
        """
